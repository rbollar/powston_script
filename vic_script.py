...
IMPORT_TOLERANCE = 7
BATTERY_SOC_NEEDED = 35
BUY_DELTA_THRESHOLD = 51
CUT_OFF_THRESHOLD = 82
MORNING_SELL_MARGIN = 87
MORNING_SELL_SOC = 40
BATTERY_EXPORT_SOC_THRESHOLD = 34
GOOD_SUN_DAY = 72
GOOD_SUN_HOUR = 7
BAD_SUN_DAY_KEEP_SOC = 21
ALWAYS_IMPORT_SOC = 11  # The battery SOC at which we always import
...
if global_tilted_irradiance_tomorrow < (GOOD_SUN_DAY * 50):
    night_reserve += BAD_SUN_DAY_KEEP_SOC * 2
...
if global_tilted_irradiance_today > (GOOD_SUN_DAY * 100):
    if interval_time.hour < 12:
        low_amber_buy_price = min(buy_forecast)
        if rrp < 0:
            action = 'charge'
        elif action in ['auto', 'charge', 'import'] and amber_sell_price > low_amber_buy_price:
            action = 'discharge'
    ...
    if interval_time.hour < 10:
        if battery_soc > MORNING_SELL_SOC:
            low_amber_buy_price = min(buy_forecast)
            if amber_sell_price > (low_amber_buy_price + MORNING_SELL_MARGIN):
                action = 'export'
                reason += f' lots of SOC, good sun and better buys coming {low_amber_buy_price}c'
            else:
                reason += f' low buy not enough {low_amber_buy_price}c'
else:
    reason += f' low PV day {global_tilted_irradiance_today:.2f}W/m2'

if 14 < interval_time.hour < 16 and battery_soc < 60 and action != 'import' and amber_buy_price < 30:
    if buy_forecast and amber_buy_price > min(buy_forecast[:6]):
...
if 4 < interval_time.hour < 16 and buy_forecast and battery_soc:
    ...
    low_amber_buy_price = round(max(sorted(buy_forecast)[:charge_fors]), 2)
    ...
    tolerant_low_price = round(low_amber_buy_price * ((100 + IMPORT_TOLERANCE) / 100), 2)
    if action in ['auto', 'charge'] and (battery_soc - ALWAYS_IMPORT_SOC) < precent_pv_past:
        if buy_forecast and amber_buy_price > min(buy_forecast[:6]) and battery_soc > 85:
            reason += ' wait for lower buy soon'
        elif amber_buy_price < tolerant_low_price:
            action = 'import'
            reason += f' buy price {amber_buy_price} is lower than {tolerant_low_price}'
        else:
            if action == 'import':
                action = 'auto'
...
if sell_forecast and action == 'export':
    # Count sells above the current amber_sell_price
    ...
    if amber_sell_price < (max_buy_forecast * BUY_DELTA_THRESHOLD / 100):
        action = 'auto'
        reason += f' sell price {amber_sell_price} is higher than buy price {max_buy_forecast}'
    elif amber_sell_price < (cut_off - CUT_OFF_THRESHOLD):
        action = 'auto'
        reason += f' sell price {amber_sell_price} is lower than 3c within {cut_off}'
    else:
        reason += f' okay to export {windows_can_export}'

if action == 'export' and battery_soc > night_reserve and interval_time > sunset and amber_buy_price < 98:
    action = 'auto'
    reason += ' in night reserve mode'
    if amber_buy_price > 25:
        action = 'export'
        solar = 'curtail'
...
if sell_forecast and buy_forecast and soc_diff_remaining > 25:
    high_amber_sell_price = max(sorted(sell_forecast)[:-3]) if sell_forecast else 0
    if high_amber_sell_price < (amber_sell_price + 5):
        action = 'export'
        reason += f' export: with 5c of high_amber_sell_price {high_amber_sell_price}c/kWh'
if sell_forecast and buy_forecast and soc_diff_remaining > 15:
    high_amber_sell_price = max(sorted(sell_forecast)[:-3]) if sell_forecast else 0
    low_amber_buy_price = min(buy_forecast) * 1.3
    if amber_sell_price > low_amber_buy_price:
        action = 'export'
        reason += f' export: sell price {amber_sell_price} is higher than low buy price {low_amber_buy_price}c/kWh'
if not daytime and time_left < 2 and soc_diff_remaining > 0:
    high_amber_sell_price = max(sorted(sell_forecast)[:-3]) if sell_forecast else 0
    if amber_sell_price > (high_amber_sell_price - 5):
        action = 'export'
        reason += f' export: sell price {amber_sell_price} is higher than high sell price {high_amber_sell_price}c/kWh'

if sell_forecast and soc_diff < 5:
    # Now we only want to export if the RRP is high enough
    high_amber_sell_price = max(sorted(sell_forecast)[:-3]) if sell_forecast else 0
    if amber_sell_price < high_amber_sell_price and action == 'export':
        action = 'auto'
        reason += f' waiting for {high_amber_sell_price}c, battery SOC {battery_soc:.1f}%'
if forecast and action == 'auto':
    is_spike_coming = max(forecast) > 1000
    reason += f' is_spike:{is_spike_coming}'
    if is_spike_coming and hour < 16 and soc_diff < 0 and amber_buy_price < 50:
        action = 'import'
        reason += ' import ahead of a spike'
    else:
        reason += f' {action} not importing {soc_diff} at {amber_buy_price}'
else:
    reason += f' {action} not importing {soc_diff} at {amber_buy_price}'
# Spike Hacking
if forecast and action == 'export':
    ...
    if rrp > 1000:
        ...
        action = 'auto'
        reason += f' not exporting, {over_count} prices over sell price {amber_sell_price}c'
    else:
